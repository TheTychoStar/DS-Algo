...
print(list1[::-1])   # -1 is step size from backward, this will make the whole list inverse
...
#Immutable objects return new instances when methods are called on them. Examples: tuples, strings.
...
#temp=(queue.popleft(),queue.popleft(),queue.popleft(),queue.popleft(),queue.popleft())
print(queue.popleft(),queue.popleft(),queue.popleft(),queue.popleft(),queue.popleft())
print(queue)
...
dr = np.random.randint (20,size=7)  ## create 7 rand int between [0, 20)
print (dr)
print (dr.dtype)
print (dr.shape)

dr = np.random.randint (3,15,size=[6,5])  ## create 6 row , 5 column numpy array , between [3, 15)
print (dr)
print (dr.dtype)
print (dr.shape)

dr = np.random.randint([0,10,100] ,[10,100,1000] ,size = [5,3]) # lower bound and upper bound could be more flexiable
print (dr)
...
plt.imshow(jpg)
#plt.imshow(jpg[::-1])   #the 1st : means no change on the 1st dimension, the -1 means step size = 1 , but reverse order
#plt.imshow(jpg[:,::-1])  #the 1st : means no change on the 1st dimension,  ::-1  means change the 2nd dimension (right <=> left)
...
a = np.array([1,2,3])
b = np.array([4,5,6])
#c = np.array([3,4])  # ValueError: operands could not be broadcast together with shapes (3,) (2,)
c = np.array([3])
print (a + b)
print (a + c)
...
y = ["PALLADIUM - NEW YORK MERCANTILE EXCHANGE","PLATINUM - NEW YORK MERCANTILE EXCHANGE",
...
#     print(df3.Market_and_Exchange_Names)
    ...
    plt.plot(df3.Report_Date_as_MM_DD_YYYY, df3.M_Money_Positions_Long_ALL, 'r.-', label='Long')
    plt.plot(df3.Report_Date_as_MM_DD_YYYY, df3.M_Money_Positions_Short_ALL,'b.-',label='Short')
    plt.plot(df3.Report_Date_as_MM_DD_YYYY, df3.M_Money_Positions_Long_ALL - df3.M_Money_Positions_Short_ALL,'g.-',label='Net')
